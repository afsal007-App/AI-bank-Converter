...
if uploaded_files:
    ...
    # Dynamically import the respective bank's script
    module_name = BANK_OPTIONS[selected_bank]
    try:
        bank_module = importlib.import_module(module_name)  # Import respective module
        df_final = bank_module.process(pdf_paths)  # Call the `process` function
